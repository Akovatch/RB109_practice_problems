...
The input is a string of digits. Cut the string into chunks (a chunk here is a substring of
...
def revrot(string, size)
  ...
  results = []
  string.chars.each_slice(size) do |arr|
    sum = 0
    next if arr.length < size
    arr.each do |digit|
      sum += digit.to_i ** 2
    end
  sum % 2 == 0 ? results << arr.reverse.join : results << arr.rotate.join
  end
  results.join
end
