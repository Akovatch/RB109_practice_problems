...
# distances are positive or zero integers and this list has at least one element).
...
# xs = [50] choose_best_sum(163, 3, xs) -> nil (or null
# or ... or -1 (C++, C, D, Rust, Swift, Go, ...)

# ys = [91, 74, 73, 85, 73, 81, 87] choose_best_sum(230, 3, ys) -> 228

# Which 3 have the highest sum that doesn't go over limit?

def choose_best_sum(max_sum, num_towns, distances)
  return nil if num_towns > distances.size
  combos = distances.combination(num_towns)
  valid_combos = combos.select { |combo| combo.sum <= max_sum }
  valid_combos.empty? ? nil : valid_combos.max_by { |combo| combo.sum }.sum
end
